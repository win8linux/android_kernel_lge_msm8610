/* Copyright (c) 2013, The Linux Foundation. All rights reserved.
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 and
 * only version 2 as published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 */
#include "msm_sensor.h"
#include <mach/board_lge.h>		//to use lge_get_board_revno()
#define HI841_SENSOR_NAME "hi841"

#define CONFIG_HI841_DEBUG
#undef CDBG
#ifdef CONFIG_HI841_DEBUG
#define CDBG(fmt, args...) pr_err(fmt, ##args)
#else
#define CDBG(fmt, args...) do { } while (0)
#endif

DEFINE_MSM_MUTEX(hi841_mut);

static struct msm_sensor_ctrl_t hi841_s_ctrl;

/* LGE_CHANGE_S, jaehan.jeong, 2013.7.30,  To separate power settings depending on HW revisions, [STARTS HERE] */
static struct msm_sensor_power_setting hi841_power_setting_rev_0[] = {
	{
		.seq_type = SENSOR_VREG,
		.seq_val = CAM_VDIG,
		.config_val = 0,
		.delay = 0,
	},
	{
		.seq_type = SENSOR_VREG,
		.seq_val = CAM_VANA,
		.config_val = 0,
		.delay = 0,
	},
	{
		.seq_type = SENSOR_VREG,
		.seq_val = CAM_VAF,
		.config_val = 0,
		.delay = 0,
	},
	{
		.seq_type = SENSOR_VREG,
		.seq_val = CAM_VIO,
		.config_val = 0,
		.delay = 0,
	},
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_RESET,
		.config_val = GPIO_OUT_LOW,
		.delay = 1,
	},
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_STANDBY,
		.config_val = GPIO_OUT_HIGH,
		.delay = 0,
	},
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_RESET,
		.config_val = GPIO_OUT_HIGH,
		.delay = 30,
	},
	{
		.seq_type = SENSOR_CLK,
		.seq_val = SENSOR_CAM_MCLK,
		.config_val = 0,
		.delay = 1,
	},
	{
		.seq_type = SENSOR_I2C_MUX,
		.seq_val = 0,
		.config_val = 0,
		.delay = 0,
	},
};

static struct msm_sensor_power_setting hi841_power_setting_on_8x10[] = {
	 /* Set GPIO_RESET to low to disable power on reset*/
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_RESET,
		.config_val = GPIO_OUT_LOW,
		.delay = 1,
	},

	{
		.seq_type = SENSOR_VREG,
		.seq_val = CAM_VIO,
		.config_val = 0,
		.delay = 1,
	},

	{										//VANA2v8, VDVDD1v2 LDO
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_VANA,
		.config_val = GPIO_OUT_HIGH,
		.delay = 1,
	},
#if !defined(CONFIG_MACH_MSM8X10_L70P)
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_STANDBY,
		.config_val = GPIO_OUT_HIGH,
		.delay = 0,
	},
#endif
#if 0 //need to check
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_VDIG,
		.config_val = GPIO_OUT_HIGH,
		.delay = 1,
	},
#endif
	{
		.seq_type = SENSOR_CLK,
		.seq_val = SENSOR_CAM_MCLK,
		.config_val = 0,
		.delay = 1, //TODO : >= 16MCLK ?
	},
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_RESET,
		.config_val = GPIO_OUT_HIGH,
		.delay = 10,
	},
	{
		.seq_type = SENSOR_I2C_MUX,
		.seq_val = 0,
		.config_val = 0,
		.delay = 0,
	},
};
/* LGE_CHANGE_E, jaehan.jeong, 2013.7.30,  To separate power settings depending on HW revisions,  [ENDS HERE] */

static struct v4l2_subdev_info hi841_subdev_info[] = {
	{
		.code = V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace = V4L2_COLORSPACE_JPEG,
		.fmt = 1,
		.order = 0,
	},
};

static const struct i2c_device_id hi841_i2c_id[] = {
	{HI841_SENSOR_NAME, (kernel_ulong_t)&hi841_s_ctrl},
	{ }
};

static int32_t msm_hi841_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	return msm_sensor_i2c_probe(client, id, &hi841_s_ctrl);
}

static struct i2c_driver hi841_i2c_driver = {
	.id_table = hi841_i2c_id,
	.probe  = msm_hi841_i2c_probe,
	.driver = {
		.name = HI841_SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client hi841_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_WORD_ADDR,
};

static const struct of_device_id hi841_dt_match[] = {
	{.compatible = "qcom,hi841", .data = &hi841_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, hi841_dt_match);

static struct platform_driver hi841_platform_driver = {
	.driver = {
		.name = "qcom,hi841",
		.owner = THIS_MODULE,
		.of_match_table = hi841_dt_match,
	},
};

// hynix i2c clock init.
static struct msm_camera_i2c_reg_array hynix_841_init_i2c_block[] = {
	{0x8408,0x0a},
	{0x0103,0x01},
	{0x0103,0x00},
	{0x8400,0x03},
};

int32_t hi841_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	uint16_t chipid = 0;

	struct msm_camera_i2c_reg_setting boot_array;
	int32_t write_rc = 0 ;

	boot_array.reg_setting = &(hynix_841_init_i2c_block[0]);
	boot_array.addr_type = MSM_CAMERA_I2C_WORD_ADDR;
	boot_array.data_type = MSM_CAMERA_I2C_BYTE_DATA;
	boot_array.delay = 0;
	boot_array.size = 4;

	write_rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_table(s_ctrl->sensor_i2c_client, &boot_array);

	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
			s_ctrl->sensor_i2c_client,
			s_ctrl->sensordata->slave_info->sensor_id_reg_addr,
			&chipid, MSM_CAMERA_I2C_WORD_DATA);
	if (rc < 0) {
		pr_err("%s: %s: read id failed\n", __func__,
			s_ctrl->sensordata->sensor_name);
		return rc;
	}

	pr_info("%s: read id: %x expected id %x:\n", __func__, chipid,
		s_ctrl->sensordata->slave_info->sensor_id);
	if (chipid != s_ctrl->sensordata->slave_info->sensor_id) {
		pr_err("hi841_sensor_match_id chip id doesnot match\n");
		return -ENODEV;
	}
	pr_err("%s:%d success!\n", __func__, __LINE__);
	return rc;
}

static int32_t hi841_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	const struct of_device_id *match;

	match = of_match_device(hi841_dt_match, &pdev->dev);

	if(!match)
		return -EINVAL;

	rc = msm_sensor_platform_probe(pdev, match->data);
	return rc;
}

static int __init hi841_init_module(void)
{
	int32_t rc = 0;
	hw_rev_type rev_type = 0;
	pr_info("%s:%d\n", __func__, __LINE__);
	rev_type = lge_get_board_revno();

	switch(rev_type) {
		case HW_REV_0:
			printk("%s: Sensor power is set as Rev.0\n", __func__);
			hi841_s_ctrl.power_setting_array.power_setting = hi841_power_setting_rev_0;
			hi841_s_ctrl.power_setting_array.size = ARRAY_SIZE(hi841_power_setting_rev_0);
			break;
		case HW_REV_A:
		case HW_REV_B:
		default:
			printk("%s: HW rev is %d. Sensor power is set as hi841_power_setting_on_8x10 \n", __func__,rev_type);
			hi841_s_ctrl.power_setting_array.power_setting = hi841_power_setting_on_8x10;
			hi841_s_ctrl.power_setting_array.size = ARRAY_SIZE(hi841_power_setting_on_8x10);
			break;
	}

	rc = platform_driver_probe(&hi841_platform_driver,
		hi841_platform_probe);
	if (!rc)
		return rc;
	pr_err("%s:%d rc %d\n", __func__, __LINE__, rc);
	return i2c_add_driver(&hi841_i2c_driver);
}

static struct msm_sensor_fn_t hi841_sensor_func_tbl = {
	.sensor_config = msm_sensor_config,
	.sensor_power_up = msm_sensor_power_up,
	.sensor_power_down = msm_sensor_power_down,
	.sensor_match_id = hi841_sensor_match_id, //jongkwon.chae@lge.com
};

static void __exit hi841_exit_module(void)
{
	pr_info("%s:%d\n", __func__, __LINE__);

	if (hi841_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&hi841_s_ctrl);
		platform_driver_unregister(&hi841_platform_driver);
	} else {
		i2c_del_driver(&hi841_i2c_driver);
	}
	return;
}

static struct msm_sensor_ctrl_t hi841_s_ctrl = {
	.sensor_i2c_client = &hi841_sensor_i2c_client,
	//.power_setting_array.power_setting = hi841_power_setting,
	//.power_setting_array.size = ARRAY_SIZE(hi841_power_setting),
	.msm_sensor_mutex = &hi841_mut,
	.sensor_v4l2_subdev_info = hi841_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(hi841_subdev_info),
	.func_tbl = &hi841_sensor_func_tbl,
};

module_init(hi841_init_module);
module_exit(hi841_exit_module);
MODULE_DESCRIPTION("hi841");
MODULE_LICENSE("GPL v2");
